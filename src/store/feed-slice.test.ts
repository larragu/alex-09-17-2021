...
describe('Feed slice', () => {
  ...
  test('should output transformed feed', () => {
    store.dispatch(
      feedActions.processSocketData({ bids: initialFilteredFeed.bids })
    );
    store.dispatch(
      feedActions.processSocketData({ asks: initialFilteredFeed.asks })
    );
  ...
  test('should change market to ETHEREUM', () => {
    store.dispatch(
      feedActions.changeMarket({ selectedMarket: Market.ETH_USD })
    );
