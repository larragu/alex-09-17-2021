...
export const transformData = (
  ...
  const filteredOrders: Order[] = originalFilteredOrders.map(
    (el: number[]) => ({ price: el[PRICE], size: el[SIZE] })
  );
    ...
    if (Object.keys(map).length >= MAX_SIZE) {
      ...
      if (sortOption === SortOption.DESCENDING) {
        if (!map[filt.price] && filt.price > lowestHighestBidAskPrice) {
          lowestHighestBidAskPrice = deleteAndAdd(
            filt,
            lowestHighestBidAskPrice,
            arr,
            map,
            sortOption
          );
  ...
  return {
    feed: { depthArray: [...depthArray], list: [...arr], map: { ...map } },
    [highLow]: lowestHighestBidAskPrice,
    [highLow2]: arr[0],
  };
};
